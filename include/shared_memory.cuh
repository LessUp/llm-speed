...
struct SharedMemoryTile {
    ...
    __device__ __forceinline__ T& operator()(int row, int col) { return data[row][col]; }

    __device__ __forceinline__ const T& operator()(int row, int col) const {
        return data[row][col];
    }

    __device__ __forceinline__ T* row_ptr(int row) { return data[row]; }
};

// Coalesced load from global memory to shared memory
template<typename T, int BLOCK_SIZE>
__device__ __forceinline__ void load_tile_coalesced(const T* __restrict__ global_ptr,
...
template<typename T, int BLOCK_SIZE>
__device__ __forceinline__ void load_tile_coalesced_bounded(const T* __restrict__ global_ptr,
                                                            T* __restrict__ smem_ptr,
                                                            int global_stride,
                                                            int smem_stride,
                                                            int rows,
...
template<typename T, int BLOCK_SIZE>
__device__ __forceinline__ void store_tile_coalesced(const T* __restrict__ smem_ptr,
...
__device__ __forceinline__ void load_float4(const float* __restrict__ src,
...
__device__ __forceinline__ void store_float4(const float* __restrict__ src,
                                             float* __restrict__ dst) {
...
__device__ __forceinline__ void load_half2(const half* __restrict__ src, half* __restrict__ dst) {
