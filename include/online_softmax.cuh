...
struct OnlineSoftmaxState {
    ...
    __device__ __forceinline__ OnlineSoftmaxState(float m, float l) : max_val(m), sum_exp(l) {}
};

// Update online softmax state with a new value
__device__ __forceinline__ void online_softmax_update(OnlineSoftmaxState& state, float new_val) {
...
__device__ __forceinline__ void online_softmax_update_block(OnlineSoftmaxState& state,
                                                            const float* values,
                                                            int count) {
    // Find max in block
...
__device__ __forceinline__ OnlineSoftmaxState online_softmax_merge(const OnlineSoftmaxState& a,
...
__device__ __forceinline__ float online_softmax_finalize(const OnlineSoftmaxState& state) {
...
__device__ __forceinline__ float online_softmax_weight(float value,
...
__device__ __forceinline__ void online_softmax_update_output(float* output,
                                                             const float* v_block,
