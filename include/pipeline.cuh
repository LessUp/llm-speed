...
public:
    ...
    __device__ __forceinline__ PipelineScheduler() : current_stage(0), compute_stage(0) {
#pragma unroll
    ...
    __device__ __forceinline__ int get_load_stage() const { return current_stage; }

    __device__ __forceinline__ int get_compute_stage() const { return compute_stage; }

    __device__ __forceinline__ void* get_load_buffer() { return buffers[current_stage].smem_ptr; }
...
struct DoubleBuffer {
    ...
    __device__ __forceinline__ DoubleBuffer(T* buf0, T* buf1) : current(0) {
        buffer[0] = buf0;
        buffer[1] = buf1;
    }

    __device__ __forceinline__ T* get_load_buffer() { return buffer[current]; }

    __device__ __forceinline__ T* get_compute_buffer() { return buffer[1 - current]; }

    __device__ __forceinline__ void swap() { current = 1 - current; }
};
...
__device__ __forceinline__ void async_copy(void* dst, const void* src) {
    ...
    if constexpr (BYTES == 16) {
        asm volatile("cp.async.ca.shared.global [%0], [%1], 16;\n" ::"r"(
                         static_cast<unsigned>(reinterpret_cast<uintptr_t>(dst))),
                     "l"(src));
...
__device__ void software_pipeline(int num_iterations, LoadFunc load_fn, ComputeFunc compute_fn) {
