...
void naive_attention_fp32(const float*,
                          const float*,
                          const float*,
                          float*,
                          int,
...
void tiled_attention_fp32(const float*,
                          const float*,
                          const float*,
                          float*,
                          int,
...
void tiled_attention_fp16(
    const half*, const half*, const half*, half*, int, int, int, int, float, bool, cudaStream_t);
void flash_attention_fp32(const float*,
                          const float*,
                          const float*,
                          float*,
                          int,
...
void tensor_core_gemm_fp16(
    const half*, const half*, float*, int, int, int, float, float, cudaStream_t);
void hgemm_fp32(const float*,
                const float*,
                float*,
                int,
                int,
...
void hgemm_fp16(const half*,
                ...
                MatrixLayout,
                cudaStream_t);
void tensor_core_gemm_int8(const int8_t*, const int8_t*, int32_t*, int, int, int, cudaStream_t);
...
void validate_gemm_inputs(const torch::Tensor& a,
    ...
    // Validate tensor dimensions are positive
    TORCH_CHECK(a.size(0) > 0 && a.size(1) > 0, "A tensor dimensions must be positive (non-zero)");
...
torch::Tensor tensor_core_gemm(const torch::Tensor& a,
                               const torch::Tensor& b,
                               float alpha = 1.0f,
                               float beta = 0.0f) {
    ...
    tensor_core_gemm_fp16(reinterpret_cast<const half*>(a.data_ptr<at::Half>()),
                          reinterpret_cast<const half*>(b.data_ptr<at::Half>()),
                          output.data_ptr<float>(),
                          M,
                          N,
...
torch::Tensor tensor_core_gemm_int8_wrapper(const torch::Tensor& a, const torch::Tensor& b) {
    ...
    tensor_core_gemm_int8(reinterpret_cast<const int8_t*>(a.data_ptr<int8_t>()),
...
PYBIND11_MODULE(cuda_llm_ops, m) {
    ...
    m.def("tensor_core_gemm_int8",
          &tensor_core_gemm_int8_wrapper,
          py::arg("a"),
          py::arg("b"),
