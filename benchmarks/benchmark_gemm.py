            ...
            if dtype == torch.float16:
                try:
                    ...
                    result["tensor_core_relative"] = (
                        result["tensor_core_tflops"] / result["cublas_tflops"]
                    )
                except Exception as e:
...
def print_results(results: List[Dict]):
    ...
    print(
        f"{'(M x N x K)':>20} | {'(ms)':>12} | {'(ms)':>12} | {'(ms)':>12} | {'of cuBLAS':>10} | {'of cuBLAS':>10}"
    )
    print("-" * 100)
    ...
    for r in results:
        ...
        print(
            f"{size_str:>20} | {cublas_ms:>12.3f} | {custom_ms:>12.3f} | {tc_str:>12} | {custom_rel:>9.1f}% | {tc_rel_str:>10}"
        )
...
def main():
    ...
    results = benchmark_gemm(
        sizes=sizes, dtype=dtype, warmup=args.warmup, iterations=args.iterations
    )
