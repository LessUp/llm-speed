...
template<typename T,
         ...
         >
__global__ void hgemm_register_tiled_kernel(const T* __restrict__ A,
...
#define COMPUTE_TILE(buf)                                          \
    _Pragma("unroll") for (int k = 0; k < BLOCK_K; k++) {          \
        _Pragma("unroll") for (int m = 0; m < THREAD_M; m++) {     \
            reg_A[m] = smem_A[buf][warp_row + thread_row + m][k];  \
        }                                                          \
        _Pragma("unroll") for (int n = 0; n < THREAD_N; n++) {     \
...
template void hgemm_impl<float>(const float*,
                                const float*,
                                float*,
                                int,
                                int,
...
void gemm_nn(const float* A,
             ...
             cudaStream_t stream) {
    hgemm_fp32(
        A, B, C, M, N, K, alpha, beta, MatrixLayout::RowMajor, MatrixLayout::RowMajor, stream);
}
...
void gemm_nt(const float* A,
             ...
             cudaStream_t stream) {
    hgemm_fp32(
        A, B, C, M, N, K, alpha, beta, MatrixLayout::RowMajor, MatrixLayout::ColMajor, stream);
}

void gemm_tn(const float* A,
             const float* B,
             float* C,
             int M,
             int N,
             ...
             cudaStream_t stream) {
    hgemm_fp32(
        A, B, C, M, N, K, alpha, beta, MatrixLayout::ColMajor, MatrixLayout::RowMajor, stream);
}

void gemm_tt(const float* A,
             const float* B,
             float* C,
             int M,
             int N,
             ...
             cudaStream_t stream) {
    hgemm_fp32(
        A, B, C, M, N, K, alpha, beta, MatrixLayout::ColMajor, MatrixLayout::ColMajor, stream);
}
