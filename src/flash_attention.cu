...
#define SMEM_V(buf) (smem_V_buf + (buf) * kv_buf_size)
    ...
    for (int i = tid; i < BLOCK_M * head_dim; i += blockDim.x) {
        ...
        smem_Q[m * hd_stride + d] =
            (global_row < seq_len) ? static_cast<float>(q_ptr[global_row * head_dim + d]) : 0.0f;
    }
    __syncthreads();
...
#define LOAD_KV_TILE(buf_idx, col_start_val)                                                      \
    for (int i = tid; i < BLOCK_N * head_dim; i += blockDim.x) {                                  \
        ...
        float kval =                                                                              \
            (global_col < seq_len) ? static_cast<float>(k_ptr[global_col * head_dim + d]) : 0.0f; \
        float vval =                                                                              \
            (global_col < seq_len) ? static_cast<float>(v_ptr[global_col * head_dim + d]) : 0.0f; \
    ...
    for (int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {
        ...
        if (is_causal && has_next) {
            ...
            if (next_col_start >= row_start + BLOCK_M)
                has_next = false;
        }
        if (has_next) {
            int next_col_start = next_kv_block * BLOCK_N;
        ...
        if (tid < BLOCK_M) {
            ...
            if (global_row < seq_len) {
                ...
                rescale[m] =
                    (old_sum > 0.0f && new_sum > 0.0f) ? (old_scale_f * old_sum / new_sum) : 0.0f;
...
// Host wrappers
void flash_attention_fp32(const float* Q,
                          const float* K,
                          const float* V,
                          float* O,
                          int batch_size,
    ...
    if (static_cast<int>(smem_size) > max_smem) {
        throw std::runtime_error("flash_attention: head_dim=" + std::to_string(head_dim) +
...
void flash_attention_fp16(const half* Q,
                          const half* K,
                          const half* V,
                          half* O,
                          int batch_size,
    ...
    int hd_stride = head_dim + 1;
    int sn_stride = BLOCK_N + 1;
    size_t smem_size = (BLOCK_M * hd_stride + 4 * BLOCK_N * hd_stride  // double buffered K/V
                        + BLOCK_M * sn_stride + BLOCK_M * head_dim + 3 * BLOCK_M) *
                       sizeof(float);
